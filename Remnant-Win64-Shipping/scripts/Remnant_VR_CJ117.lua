...
local function DialDet()

	local skeletal_mesh_c = api:find_uobject("WidgetBlueprintGeneratedClass /Game/UI/UI_Dialogue/Widget_Dialogue.Widget_Dialogue_C")
    if skeletal_mesh_c ~= nil then
        ...
        for i, mesh in ipairs(skeletal_meshes) do
            if string.find(mesh:get_full_name(), "Widget_Dialogue_") and string.find(mesh:get_full_name(), "Transient") then
				--local is_vis = bVisible
                --print(tostring(mesh:get_full_name()))
				if mesh.PromptVisible == true then
					in_dialog = true
				else
					in_dialog = false
				end
                --print(tostring(mesh.PromptVisible))
                break
			else
				in_dialog = false
			end
...
local function ShowArms()
	...
	if apawn ~= nil and not string.find(apawn:get_full_name(), "MainMenu") then
		apawn.Visual_Body:call("SetRenderInMainPass", true)
	end
end
...
local function HideArms()
	local apawn = api:get_local_pawn(0)
	if apawn ~= nil and not string.find(apawn:get_full_name(), "MainMenu") then
		apawn.Visual_Body:call("SetRenderInMainPass", false)
	end
end
...
local function ShowHead()
	...
	if apawn ~= nil and not string.find(apawn:get_full_name(), "MainMenu") then
		...
		apawn.Visual_Eyebrows:call("SetRenderInMainPass", true)
		apawn.Visual_Hair:call("SetRenderInMainPass", true)
		apawn.Visual_FacialHair:call("SetRenderInMainPass", true)
		apawn.Visual_Mustache:call("SetRenderInMainPass", true)
		apawn.Visual_Helmet:call("SetRenderInMainPass", true)
	end
end
...
local function HideHead()
	...
	if apawn ~= nil and not string.find(apawn:get_full_name(), "MainMenu") then
		...
		apawn.Visual_Eyebrows:call("SetRenderInMainPass", false)
		apawn.Visual_Hair:call("SetRenderInMainPass", false)
		apawn.Visual_FacialHair:call("SetRenderInMainPass", false)
		apawn.Visual_Mustache:call("SetRenderInMainPass", false)
...
local function HideWeap()
	local cwpawn = api:get_local_pawn(0)
	
	if cwpawn:GetCurrentRangedWeapon() ~= nil then
		equipped_weapon = cwpawn:GetCurrentRangedWeapon()
		equipped_weapon:SetInHand(false)
	end	
end

local function ShowWeap()
	local cwpawn = api:get_local_pawn(0)
...
uevr.sdk.callbacks.on_pre_engine_tick(function(engine, delta)
	...
	if pawn ~= nil then
		...
		if pcont.MyHUD.ContextActor ~= nil then
			if pcont.MyHUD.ContextActor.StateMachine.CurrentState ~= nil then
				cur_HUD_state_name = tostring(pcont.MyHUD.ContextActor.StateMachine.CurrentState.NameID)
			end	
		end
	...
	if cur_state_m ~= nil and string.find(cur_state_m, "InteractiveState") then
		...
		if pcont.MyHUD.ContextActor ~= nil and cur_HUD_state_name ~= nil then
			if string.find(cur_HUD_state_name, "Occupied") then
				is_interact = true
			else
				is_interact = false
			end
	...
	if pawn == nil or string.find(pawn:get_full_name(), "MainMenu") or is_mouse == true or is_hidden == true or is_loading == true or is_cut == true or in_travel == true or is_reset == false or is_interact == true then 
		...
		if is_cut == true or in_talk == true then
			UEVR_UObjectHook.set_disabled(true)
			ShowArms()
			ShowHead()
			Third_P()
		...
		elseif string.find(cur_cam, "Vault") or string.find(cur_c_state, "Vault") then
			params.vr.set_mod_value("VR_CameraForwardOffset", "40.00")
			HideWeap()
			--equipped_weapon:SetInHand(false)
			cam_change = true
			ShowArms()	
